...
import static hu.unideb.fitbase.commons.path.gym.GymPath.GYM_CREATE;
import static hu.unideb.fitbase.commons.path.gym.GymPath.GYM_LIST_USER;
...
public class GymRestController {
    ...
    public ResponseEntity<?> postGym(@RequestBody GymRequest gymRequest) throws ViolationException {
        ...
        try {
            ...
            result = ResponseEntity.accepted().body(new SuccesResponse(gym, null));
    ...
    @GetMapping(path = GYM_LIST_USER)
    public ResponseEntity getUsersGyms() throws ViolationException {
        List<Gym> gymByUser = gymService.findUsersGym(getUser());
        return ResponseEntity.accepted().body(new SuccesResponse(gymByUser,new MetaResponse(null)));
    }
